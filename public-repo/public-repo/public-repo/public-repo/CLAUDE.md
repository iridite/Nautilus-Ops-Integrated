...
### 1. 并发数据下载优化 (2026-02-22)

**问题**：串行下载 265 个交易对的数据需要 40+ 分钟，效率低下。

**解决方案**：
- 使用 `ThreadPoolExecutor` 实现并发下载
- 添加 `NAUTILUS_MAX_WORKERS` 环境变量控制并发数（默认 10）
- 为每个线程创建独立的 exchange 实例确保线程安全
- 同时优化 OHLCV 和 OI/Funding Rate 数据下载

**性能提升**：
- 优化前：265 个交易对 ~40 分钟（串行）
- 优化后：265 个交易对 ~4-8 分钟（10 并发）
- 提升：5-10x
...
# 自定义并发数
NAUTILUS_MAX_WORKERS=20 uv run main.py backtest --type high
```

**关键代码位置**：
- `utils/data_management/data_retrieval.py`: 核心实现
- `utils/data_management/data_manager.py`: 参数传递

**最佳实践**：
- 建议并发数：5-20（取决于网络和 API 限流）
- 过高并发可能触发交易所 API 限流
- 监控下载失败率，适当调整并发数

### 2. 数据覆盖率警告策略 (2026-02-22)

**问题**：所有覆盖率 < 95% 的交易对都触发 WARNING，导致大量噪音。新币种（如 AGLDUSDT 上市于 2023-07-29）在回测 2022-2026 时自然覆盖率低（60.7%），但仍能正常交易。

**关键发现**：
- **覆盖率检查是信息性的，不影响实际交易**
- 在有数据的时间段内，策略可以正常产生信号和执行交易
- 在没有数据的时间段，该交易对不参与回测（这是预期行为）

**解决方案**：实现分级警告策略
- **< 30%**: WARNING（数据严重不足，可能影响结果可靠性）
- **30-95%**: INFO（数据部分覆盖，新币种常见情况）
- **≥ 95%**: DEBUG（数据完整）

**示例场景**：
```
回测时间：2022-01-01 到 2026-01-01（4年 = 1461天）
AGLDUSDT：2023-07-29 上市（2.4年 = 887天）
覆盖率：887 / 1461 ≈ 60.7%
...
**关键代码位置**：
- `backtest/engine_high.py`: `_check_parquet_coverage()` 函数

**最佳实践**：
- 不要过度担心 30-95% 的覆盖率（新币种正常情况）
- 关注 < 30% 的警告（可能需要检查数据完整性）
- 根据策略调整回测时间范围（避免不必要的警告）
- 定期更新数据确保覆盖到最新日期
...
- `docs/data-download-optimization.md`: 数据下载并发优化文档
- `docs/data-coverage-check.md`: 数据覆盖率检查说明
