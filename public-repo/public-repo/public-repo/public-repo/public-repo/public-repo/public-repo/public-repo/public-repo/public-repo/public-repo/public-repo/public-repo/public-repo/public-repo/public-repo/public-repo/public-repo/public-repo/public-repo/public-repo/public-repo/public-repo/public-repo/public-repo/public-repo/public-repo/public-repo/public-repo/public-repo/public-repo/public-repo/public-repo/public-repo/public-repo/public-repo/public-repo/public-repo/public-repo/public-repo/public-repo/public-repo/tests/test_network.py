...
class TestRetryFetch(unittest.TestCase):
    ...
    def test_retry_on_network_error(self):
        """测试网络错误重试"""
        mock_func = Mock(side_effect=[ccxt.NetworkError("Connection timeout"), "success"])
    ...
    def test_retry_on_exchange_error(self):
        """测试��易所错误重试"""
        mock_func = Mock(side_effect=[ccxt.ExchangeError("Temporary error"), "success"])
    ...
    def test_rate_limit_error_delay(self):
        """测试限流错误的延迟策略"""
        mock_func = Mock(side_effect=[ccxt.ExchangeError("418 rate limit exceeded"), "success"])
    ...
    def test_rate_limit_error_with_1003_code(self):
        """测试 -1003 限流错误码"""
        mock_func = Mock(side_effect=[ccxt.ExchangeError("-1003 Too many requests"), "success"])
    ...
    def test_rate_limit_error_with_text(self):
        """测试文本形式的限流错误"""
        mock_func = Mock(side_effect=[ccxt.ExchangeError("rate limit exceeded"), "success"])
    ...
    def test_network_error_backoff(self):
        """测试网络错误的指数退避"""
        mock_func = Mock(side_effect=[ccxt.NetworkError("network error"), "success"])
    ...
    def test_custom_exceptions(self):
        ...
        with patch("time.sleep"):
            result = retry_fetch(mock_func, retries=3, delay=1.0, exceptions=(CustomError,))
    ...
    def test_backoff_factor(self):
        """测试不同的退避因子"""
        mock_func = Mock(
            side_effect=[ccxt.NetworkError("timeout"), ccxt.NetworkError("timeout"), "success"]
        )
    ...
    def test_multiple_error_types(self):
        ...
        with patch("time.sleep"):
            result = retry_fetch(
                mock_func, retries=3, delay=0.1, exceptions=(ccxt.NetworkError, ccxt.ExchangeError)
            )
