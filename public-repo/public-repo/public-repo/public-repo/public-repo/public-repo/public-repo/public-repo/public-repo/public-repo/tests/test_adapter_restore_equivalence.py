...
class TestAdapterRestoreEquivalence(unittest.TestCase):
    ...
    def _set_env(self, venue: str, inst_type: str):
        ...
        """
        trading = SimpleNamespace(venue=venue, instrument_type=inst_type, main_timeframe="1d", trend_timeframe="1d", initial_balance=100)
        ...
        logging = SimpleNamespace(level="INFO", file_level="INFO", components={}, components_only=False)
        ...
        self.adapter.env_config = SimpleNamespace(trading=trading, backtest=backtest, logging=logging)
        self.adapter.active_config = SimpleNamespace(timeframe=None, price_type=None, origination=None, overrides=None)
    ...
    def test_restore_instrument_id_matches_helper_swap_binance(self, mock_loader):
        ...
        for sym in symbols:
            with self.subTest(symbol=sym):
                expected = format_aux_instrument_id(sym, template_inst_id=None, venue=self.adapter.get_venue(), inst_type=self.adapter.env_config.trading.instrument_type)
            ...
            for tf in timeframes:
                for price_type, orig in price_orig_pairs:
                    with self.subTest(symbol=sym, timeframe=tf, price=price_type, orig=orig):
                        ...
                        expected = build_bar_type_from_timeframe(inst_id, tf, price_type=price_type, origination=orig)
                        actual = self.adapter._restore_bar_type(sym, tf, price_type=price_type, origination=orig)
                        self.assertEqual(actual, expected, f"Bar type mismatch for {sym} {tf} {price_type}/{orig}")
    ...
    def test_restore_bar_type_fallback_behavior_on_unusual_timeframe(self, mock_loader):
        ...
        for tf in weird_tfs:
            with self.subTest(timeframe=tf):
                ...
                actual = self.adapter._restore_bar_type(sym, tf, price_type="LAST", origination="EXTERNAL")
                self.assertEqual(actual, expected, f"Fallback bar type mismatch for timeframe='{tf}'")
